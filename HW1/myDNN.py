...
def MyUpdateMu(paramaters, gradients, mu):
...
def buildDNN(Neuron_Distribution):
	...
	train = theano.function(inputs=[x, y_hat, t], updates=MyUpdateMu(WB_Parameters, gradients, mu), outputs=cost)
	test = theano.function(inputs=[x], outputs=y)

	return train, test, mu_decay
...
if __name__ == "__main__" :
	...
	train, test, mu_decay = buildDNN(Neuron_Distribution)
	...
	for t in xrange(Iterations):
		...
		for i in xrange(batchNumber):
			cost += train(xBatch[i], yHatBatch[i], t)
